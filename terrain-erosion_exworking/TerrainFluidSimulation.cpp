...
void TerrainFluidSimulation::runMainloop()
{
    ...
    while(!_finished)
    {
        ...
        if (counterSim%100 == 0)
        {
            ...
            std::cout <<" pause is " << _inPause << std::endl;
            counterSim = 0;
        }
...
void TerrainFluidSimulation::checkInput()
{
    ...
    if (glfwGetKey(_window,'L')) _flood = false;
     ...
     if (glfwGetKey(_window,'U'))
     {
         _inPause = true;
     }
     if (glfwGetKey(_window,'J'))
     {
         _inPause = false;
     }
...
void TerrainFluidSimulation::ExportSimulationInObjFormat()
{
           ...
           objfile.open(filename, std::fstream::in | std::fstream::out );
            ...
            {    // use existing file
               ...
               objfile << "Appending writing and working with existing file"<<"\n---\n";
               objfile.close();
               std::cout<<"\n";
...
void TerrainFluidSimulation:: SaveTerrain (std::fstream *objfile)
{
   ...
   //_simulationState.terrain
           ...
           {
               (*objfile )<<"f " << gridIndices.at(i) << " " << gridIndices.at(i+1) << " "<< gridIndices.at(i+2) << std::endl;
           }*/
...
void TerrainFluidSimulation::init()
{
    ...
    // position camera
    _cam.TranslateGlobal(vec3(0.4f,0.8,2));
    _cam.ViewMatrix();
