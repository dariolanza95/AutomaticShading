                               ...
                               RixSCParamInfo const *ignored)
{
    ...
    *noutputs = 3;
    ...
    PtcPointCloud inptc = PtcSafeOpenPointCloudFile( _output_file_name);
        ...
        float cell_scale = 1;
        float scale = M_PI*2;// /(cell_scale/2);

        float maxdist = 340;//+ 10*(1-cell_scale);
      //  scale = scale   * 10 ;
        float *data;
        float point[3];
        float normal[3];
        ...
        float res = 0;
float proj = 0;
    RtPoint3 f1cell, f2cell,f3cell,f4cell;
        ...
        for (int i = -1;  i <= 1;  i += 1)
        {
            for (int j = -1;  j <= 1;  j += 1)
            {
                for (int k = -1;  k <= 1;  k += 1)
                {
                    ...
                    RtPoint3 testcell = thiscell + RtVector3(i*cell_scale,j*cell_scale,k*cell_scale);
                    RtPoint3 pos = testcell;// + jitter[n]*0.5*cell_scale*(RtVector3(m_sFuncs->CellNoise(testcell)) - 0.5f);
...
/*
       ...
       // F1 cell
            ...
            float res1=0,res2=0,res3=0,res4 = 0;
               ...
               /* if(f2-f3<maxdist)
                ...
                if(Readres==1)
                {
                    ...
                    res3 = sin(scale*proj);
                    float range = f2-f3;
                    res2 = res2*((range*maxdist)*0.5 + 0.5) + res3*(1-(range*maxdist)*0.5 + 0.5);
                    //res2 = modf(scale*proj,&m_pi);
                }
                }*/
               /* point[0] = f4cell.x;
         ...
         //       res2 = asin(res2);
                ...
                //if(res1-res2 < M_PI/2)
                {
                    res = res1 * (1-(f1/(f1+f2))) + res2 * (1-(f2/(f1+f2))) ;
                    //res = sin(res*scale);
                    res = res2;
                    res = 0.5 + res*0.5;

                    blend  = RixSmoothStep(0.4,0.5, res);
                    if(res == 0.5)
                        blend = -1;
                    if(res<0|| res>1)
                        std::cout<<"error in blend"<<std::endl;
                }
              /*  else
                {
                    blend = RixSmoothStep(0,.1, res1);
                }*/

                // std::cout<<res<<std::endl;
                //res = (res1 * 0.5 + res2 * 0.5);


   /*//temporary part-begin
                point[0] = pp.x;
                point[1] = pp.y;
                point[2] = pp.z;
        ...
        blend = RixSmoothStep(-0.2, 0.2, res);

    //temporary part-end --dont forget to randomize the cell point at the start*/
       ...
       // res = roundf(res);
        ...
        if (randomScale[n] != 0.f &&  index1!=index2)
        {
            ...
            RtColorRGB black(1,0,0);
            RtColorRGB white(1,1,1);

            RtColorRGB blue(0,0,1);
           resultDispl[n] = 0;if(blend!=-1)
               resultRGB[n] = RixLerpRGB(black,white,blend);
               else
                   resultRGB[n] = blue;

           /*resultRGB[n].r =resultRGB[n].g=resultRGB[n].b= 0.5+0.5*(/*
                       res1 * (1-(f1/(f1+f2)))  + res2 * (1-(f2/(f1+f2)))

                       ) ;*/
            //resultDispl[n] = blend*0.5;
          //  int r = f1cell.x;
          //  int g = f1cell.y;
          //  int b = f1cell.z;
          // if((r+g+b)% 2 == 0  )
