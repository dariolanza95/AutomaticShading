...
float PxrWorleyD::interpolatedResult()
{
    ...
    float differential_step = 1;
        ...
        if (invert != 0)
        {
            ...
            resx = 0;
                ...
                RtPoint3 yPlane_origin (0,y1,0);
                RtPoint3 zPlane_origin (0,0,z1);
                float t = 0;

               /*temppoint = xcell-testpoint;
                            dir1.x = (data[2]);
                            dir1.y = (data[3]);
                            dir1.z = (data[4]);
                            Normalize(dir1);
                            proj = Dot(dir1,temppoint);
                            resx= sin(scale*proj);
                            */

                RtVector3 temp_vector = Cross(dir4,RightDir);
                ...
                if(Readres1==1)
                {
                    ...
                    if(t!=-1)
                    {
                        ...
                        temppoint = testpoint-thiscell;
                        if(temppoint.x<=cell_scale && temppoint.x>= -cell_scale)
                        {
                           /* temppoint = xcell-testpoint;
                            dir1.x = (data[2]);
                            dir1.y = (data[3]);
                            ...
                            proj = Dot(dir1,temppoint);
                            resx= sin(scale*proj);*/
                            temppoint =  thiscell- testpoint ;
                            proj = Dot(dir4,temppoint);
                    ...
                    dir1.z = (data[4]);
                    Normalize(dir1);
                    temp_vector = Cross(dir1,RightDir);
